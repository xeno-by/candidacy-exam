...
\item Do it in their own interesting ways
\end{itemize}
\end{frame}

\begin{frame}[fragile]
\frametitle{Template Haskell: The \texttt{aif} macro}
\begin{semiverbatim}
...
\visible<2->{\text{\color{blue}{Not in scope: `it'}}}
\end{semiverbatim}
\end{frame}
...
\begin{frame}[fragile]
\frametitle{Nemerle: The \texttt{aif} macro}
\begin{semiverbatim}
aif(calculate,
...
\visible<2->{\text{\color{blue}{error: unbound name `it'}}}
\end{semiverbatim}
\end{frame}
...
\frametitle<5>{Nemerle: Coloring algorithm}%: inherit use site}
\begin{semiverbatim}
