...
\visible<3->{(defmacro aif args}
  \visible<4->{\only<4>{(list 'let* (list (list 'temp (car args))}}\only<5->{\alert{`}(let* ((temp \only<5>{...........)}\only<6->{\alert{,(car args)})}}
          \visible<4->{\only<4>{          (list 'it 'temp))}\only<5->{(it temp))}}
     \visible<4->{\only<4>{(list 'if 'temp}\only<5->{(if temp}}
       \visible<4->{\only<4>{(cadr args)}\only<5>{............}\only<6->{\alert{,(cadr args)}}}
       \visible<4->{\only<4>{(caddr args)))}\only<5>{............}\only<6->{\alert{,(caddr args)}})}
