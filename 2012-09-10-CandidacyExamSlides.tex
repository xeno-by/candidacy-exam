...
\begin{semiverbatim}
aif if' then' else' =
  [| let \text{\color<2->{blue}{temp = $if'}}
         \text{\color<2->{blue}{it = temp}}
     in if temp \text{\color<2->{red}{/=}} 0 then $then' else $else' |]

\visible<2>{aif :: Q Exp -> Q Exp -> Q Exp -> Q Exp}
\visible<2>{aif if'' then'' else'' =}
    \visible<2>{do \{ \text{\color{blue}{temp <- newName "temp"}}}
       ...
       \visible<2>{; \text{\color{red}{let notEq = mkNameG_v "ghc-prim" "GHC.Classes" "/="}}}
         \visible<2>{in return}
          \visible<2>{(LetE [ValD (VarP temp) (NormalB if') [],}
...
\begin{itemize}
\item Quasiquotes in Template Haskell are automatically hygienic
\item They are translated into Q monad, which takes care of \texttt{gensym}ming
