...
the paper this time period is referred to as \emph{compile-time}, despite that strictly speaking interpreted
...
A lot of macro generated code comes from combining quasiquotes, and quasiquotes
are just snippets of code in target language (possibly with "holes").
Therefore it might be a good idea to typecheck quasiquotes before using them.
...
This strategy is employed by MetaML \cite{taha99}, a statically typechecked
...
A strategy employed in Template Haskell \cite{sheard02} (Figure 3b) doesn't statically ensure
that quasiquotes play well together. Unlike in MetaML, the type of syntax objects is non-polymorphic -
\texttt{Expr}, not \texttt{Expr 'a}. Therefore Template Haskell cannot guarantee that meta-programs
produce well-typed code. For example, well-typedness of a \texttt{printf} expansion is checked
on the spot, immediately after the expansion, not ahead of time.
...
Even this lightweight typecheck has proven to impose unnecessary limitations.
For example, the meta-program \texttt{f} from Figure 3b
that uses another meta-program \texttt{t} to insert a type declaration in a quasiquote is rejected up front.
This happens because the compiler cannot prove that the usage of this type declaration (\texttt{MkT x})
is correct for all possible \texttt{t}s, despite that it might actually be correct.

Therefore Nemerle \cite{skalski05, skalski04}, a macro-enabled language inspired by Template Haskell,
