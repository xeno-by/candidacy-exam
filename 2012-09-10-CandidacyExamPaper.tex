...
% i.e. on preventing syntactic macros from expanding into nonsense
% and finding synergies with existing language features.
namely on ways to prevent syntactic macros from expanding into nonsense.
...
explicitly passes special identifiers between call sites. Both approaches however involve boilerplate.
...
because the dynamic variable visible though scopes is explicitly introduced as such
and is easily discoverable, since it comes together with the originating macro.
...
Macro expansions can go wrong in several ways. The simple one is introduction of type errors,
% because of malformed syntax objects, or individually well-typed syntax objects not playing well together.
the more sophisticated one is mix up of lexical scopes.

Evolution of Lisp identified the problems that lead to
distortion of scopes and formulated the principles of hygiene and referential transparency
that prevent such errors. This gave rise to manual \cite{clinger91a,hoyte08} and automatic
techniques \cite{kohlbecker86, clinger91b, dybvig92} that increase robustness of macros.
Among languages studied in this paper, Template Haskell derives monadic representations
for quasiquotes and automatically alpha-renames introduced binders \cite{sheard01}. It is
however incapable of breaking hygiene per programmer's request without dropping
to a low-level notation. Nemerle \cite{skalski04,skalski05} and Racket \cite{barzilay11,flatt10}
use approaches descended from Dybvig's algorithm \cite{dybvig92}, which provides scope isolation by default
and mechanisms for manual control.

Type checking algorithms that catch errors in meta-programs are pioneered by MetaML
\cite{taha99,ganz01,taha03}.
...
restrictive. Therefore Nemerle doesn't type check quasiquotes at all, relying on a mandatory typecheck
after macro expansion.

\section{Research proposal}

We implemented a macro system for the Scala programming language \cite{burmako12}
and integrated it into a production version of the Scala compiler.

The main contribution of our work is bootstrapping of a minimalistic non-hygienic macro system
without quasiquotes into a hygienic extension with high-level means of building syntax objects.

The \small \texttt{reify} \normalsize macro that implements the bootstrapping is based on a notion
similar to MacroML's \cite{ganz01}, which forces the programmers to resort to low-level syntax manipulations
when untypeable templates or destructuring are involved.
Our experience and user reports show a clear need for a full-fledged hygienic quasiquoting facility.
Therefore we plan to resolve the issues with \small \texttt{reify} \normalsize while retaining as much
type safety as possible.

Another direction of research is integration with Scala's rich type system.
For example, our early adopters have suggested that the combination of vanilla macros, type macros
and path-dependent types can be used to contain and propagate effects. One more conjecture is that
