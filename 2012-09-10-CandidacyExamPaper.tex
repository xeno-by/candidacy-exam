...
\begin{adjustwidth}{2.5cm}{0pt}
a) Strict typechecking \cite{taha99}: each quasiquote is typechecked in isolation and is assigned\\
a "code of something" type (\texttt{<`a>} stands for code that evaluates to a value of type \texttt{`a}).\\
This means that quasiquotes cannot contain unbound free variables, which in most cases\\
renders the macro system \cite{ganz01} incapable of manipulating bindings.
\end{adjustwidth}
...
\begin{adjustwidth}{2.5cm}{0pt}
b) Relaxed typechecking \cite{sheard02}: quasiquotes are sanity checked to prevent blatant errors\\
...
\begin{adjustwidth}{2.5cm}{0pt}
c) Lenient typechecking \cite{skalski05, skalski04}: quasiquotes are not typechecked at all until a
macro\\ that uses them expands, after which the result, which contains a mix of quasiquotes and\\
generated code, is typechecked as a whole. This provides maximum freedom, permitting\\
even unquotes in binding positions (like in \texttt{def \$name}).
\end{adjustwidth}
