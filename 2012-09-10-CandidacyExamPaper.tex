...
(let ((let print))
  (let ((temp true))
    (if temp temp false)))
\end{verbatim}\\
b) Violation of hygiene \cite{kohlbecker86}: variable created during macro expansion
shadows binding established at the expansion site
&
c) Violation of referential transparency \cite{dybvig92}: variable used in macro expansion
changes its meaning at the macro use site
\end{tabular}
\end{listing}
\end{figure*}
